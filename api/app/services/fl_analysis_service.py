...
def execute_job(job_id: str) -> None:
    ...
    try:
        ...
        profile_rows = db.execute(
            select(TowerProfile).where(TowerProfile.tower_id.in_([tower.id for tower in towers]))
        ).scalars().all()
        ...
        _finish_placeholder_run(db, job_id=job.id, run_id=run.id, started_perf=started_perf)
    except Exception as exc:
        db.rollback()
...
def _finish_placeholder_run(db: Session, *, job_id: str, run_id: str, started_perf: float) -> None:
    job = get_job_by_id(db, job_id)
    ...
    message = _placeholder_message_for_adapter(job.external_adapter)
    now = utcnow()
