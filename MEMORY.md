...
- 生产若通过公网 `IP:3000` 暴露前端，而公网 `8000` 不可直达，则应由入口反代统一承接：浏览器端对 loopback API 基址一律回落到当前站点 origin，`/api/*` 与 `/api/v1/ws` 通过部署层反代到 `api:8000`，避免浏览器直接访问 `host:8000`。
...
## 线路管理电压等级口径（2026-05-03）

- `/admin/power-lines` 的“新建/编辑线路”中，电压等级输入已统一为下拉选择（不再允许自由数值输入）。
