...
- 根因：
  - Flower 侧对 `/api/workers?...&workername=...` 在当前版本会返回 `404 Unknown worker`，即使 `/api/tasks?workername=...` 与 `/api/workers?status=true` 正常，后端代理将其映射为 `502`。
...
- 验证：
  ...
  - 运行验证（当前 fquiz 容器）：
    ...
    - 以 `get_user_authorization` 触发鉴权路径后，DB 日志窗口内未出现 `user_role_rela/menu does not exist`。
